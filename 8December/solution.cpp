...
size_t second_solution8(){
    ...
    std::set<std::pair<size_t, size_t>> already_counted;
    for(auto const& itr : node_positions){
        already_counted.insert(itr.first);
        for(auto const& second_itr : node_positions){
            if(itr == second_itr || itr.second != second_itr.second) continue;
            size_t y;
            size_t x;
            ...
            while(y >= 0 && y < data.size() && x >= 0 && x < data[0].size()){
                already_counted.insert({y, x});
                if(itr.first.first > second_itr.first.first) y += itr.first.first - second_itr.first.first; 
                else y -= second_itr.first.first - itr.first.first;
                if(itr.first.second > second_itr.first.second) x += itr.first.second - second_itr.first.second; 
                else x -= second_itr.first.second - itr.first.second;
            }
            if(y >= 0 && y < data.size() && x >= 0 && x < data[0].size()) already_counted.insert({y, x});
        }
    }
